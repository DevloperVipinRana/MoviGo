...
function to24Hour(timeStr = "00:00", ampm = "") {
  ...
  // If hour is already in 24h format (13..23), ignore AM/PM and keep as is.
  if (h > 23) {
    // invalid hour, clamp to 23:59 fallback
    h = 23;
  } else if (h >= 13 && !a) {
    // already 24h format, no conversion
  } else if (a === "AM") {
    if (h === 12) h = 0;
  } else if (a === "PM") {
    if (h >= 1 && h <= 11) h = h + 12;
    // if h is already 12 or >12 (24h input), leave as-is
  }
...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    ...
    (async function fetchMovie() {
      ...
      // Check cache first
      const cacheKey = `movie_detail_${movieIdParam}`;
